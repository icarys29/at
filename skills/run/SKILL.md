...
argument-hint: "[deliver|triage|review|ideate] [--session <id|dir>] [--dry-run] [--from-phase <phase>|--gate <gate>] [--rollback <id|dir> [--checkpoint <cp-id>]] <request>"
...
1) Create or resume a session for the chosen workflow:
   ...
   - Run: `uv run "${CLAUDE_PLUGIN_ROOT}/scripts/session/create_session.py" --workflow "<deliver|triage|review|ideate>" [--resume <id|dir>]`
...
10) Deliver: gates (binary done):
    ...
    - If plan includes `type=lsp` verifications and `.claude/project.yaml` has `lsp.enabled=true`: Task `lsp-verifier` → writes `quality/lsp_verifications.{md,json}`
