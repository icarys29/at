...
**Bisection approach:**
1. Run first half of tests, check for pollution
2. If pollution present, bisect that half
3. Continue until single test identified
...
Trace backwards
    |
    v
Is this the source? --no--> [loop back to trace backwards]
    |
   yes
    |
    v
...
When documenting in `analysis/root_cause.md`:

```markdown
## Trace Chain

1. Symptom: [what was observed]
2. Immediate cause: [code that directly failed]
3. Call chain:
   - [function A] called with [value]
   - [function B] called with [value]
   - ... (continue tracing)
4. Original trigger: [where bad value originated]

## Root Cause

[One sentence: the actual source of the problem]

## Evidence

- [Log output / stack trace / git diff showing the trace]
```
