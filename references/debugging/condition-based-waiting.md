...
## Integration with Test Fixes

When proposing test fixes in `analysis/root_cause.md`:
...
await waitFor(
  () => getResult() === expected,
  'result to match expected value',
  5000
);
...
### Rationale
- Current: Assumes 100ms is enough; fails under load
- Proposed: Waits for actual condition; timeout only on real failure
```
