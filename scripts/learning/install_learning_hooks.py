...
# ///
"""
at: Install learning hooks (SessionStart snippet) - opt-in, idempotent
...
def _managed_hook(command: str, timeout: int) -> dict[str, Any]:
    return {
        "hooks": [
            {
                "type": "command",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Install at learning hooks into project or user settings (idempotent).")
    ...
    sess_start.append(_managed_hook(f"uv run \"{plugin_root}/scripts/hooks/sessionstart_learning_context.py\"", timeout=15))
