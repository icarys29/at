...
# ///
"""
at: Uninstall docs-keeper hooks (best-effort, idempotent)

Removes hooks installed by scripts/hooks/install_docs_keeper_hooks.py.
...
def _prune_event(items: Any) -> list[dict[str, Any]]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Remove docs-keeper hooks from project/team/user settings (idempotent).")
    ...
    for ev in ("PreToolUse", "SubagentStop"):
        if ev in hooks_cfg:
            hooks_cfg[ev] = _prune_event(hooks_cfg.get(ev))
